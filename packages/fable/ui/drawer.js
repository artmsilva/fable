...
class FableDrawer extends LitElement {
  ...
  static stories = {
    Default: (args) => html`
      <fable-button @click=${(e) => { const drawer = e.target.nextElementSibling; drawer.open = true; }}>Open Drawer</fable-button>
      <fable-drawer .open=${args.open} position=${args.position} width=${args.width}>
    ...
    "Bottom Position": (_args) => html`
      <fable-button @click=${(e) => { const drawer = e.target.nextElementSibling; drawer.open = true; }}>Open Bottom Drawer</fable-button>
    ...
    "With Code": (_args) => html`
      <fable-button @click=${(e) => { const drawer = e.target.nextElementSibling; drawer.open = true; }}>View Source</fable-button>
  ...
  constructor() { super(); this.open = false; this.position = "right"; this.width = "500px"; }
  _updateBackdrop() { if (this.open) { this._showBackdrop(); } else { this._hideBackdrop(); } }
  _showBackdrop() { if (!this._backdrop) { this._backdrop = document.createElement("div"); this._backdrop.className = "backdrop"; this._backdrop.addEventListener("click", () => this.close()); document.body.appendChild(this._backdrop); } requestAnimationFrame(() => { this._backdrop.classList.add("visible"); }); }
  _hideBackdrop() { if (this._backdrop) { this._backdrop.classList.remove("visible"); } }
  disconnectedCallback() { super.disconnectedCallback(); if (this._backdrop) { this._backdrop.remove(); this._backdrop = null; } }
  close() { this.open = false; this.dispatchEvent(new CustomEvent("close", { bubbles: true, composed: true })); }
  render() {
    return html`
  ...
  updated(changedProperties) { super.updated(changedProperties); if (changedProperties.has("width") || changedProperties.has("position")) { if (this.position === "bottom") { this.style.height = this.width; this.style.width = ""; } else { this.style.width = this.width; this.style.height = ""; } } if (changedProperties.has("open")) { this._updateBackdrop(); } }
}

define("fable-drawer", FableDrawer);
