...
class FableSelect extends LitElement {
  ...
  static properties = { label: { type: String }, value: { type: String }, disabled: { type: Boolean } };

  static args = { label: "Choose option", value: "option2", disabled: false };
  static argTypes = { disabled: { control: "boolean" } };
  static slots = {
    default: html`
...
- Provide visible focus styles and sufficient contrast.`,
    ...
    Default: (args, slots) => html`<fable-select label=${args.label} .value=${args.value} ?disabled=${args.disabled}>${slots?.default}</fable-select>`,
    "With Countries": (_args) => html`<fable-select label="Select Country" value="us">
    ...
    "Many Options": (_args) => html`<fable-select label="Choose Framework" value="react">
    ...
    Disabled: {
      ...
      render: (args, slots) => html`<fable-select label=${args.label} .value=${args.value} ?disabled=${args.disabled}>${slots?.default}</fable-select>`,
    },
  };

  constructor() { super(); this.label = ""; this.value = ""; this.disabled = false; this._options = []; }
  firstUpdated() { this._updateOptions(); }
  updated(changedProperties) { super.updated(changedProperties); const selectEl = this.shadowRoot?.querySelector("select"); if (selectEl && this.value) { selectEl.value = this.value; } }
  _handleSlotChange() { this._updateOptions(); }
  _updateOptions() { requestAnimationFrame(() => { const options = Array.from(this.querySelectorAll("fable-select-option")); this._options = options.map((opt) => ({ value: opt.value || "", label: opt.label || opt.textContent?.trim() || "" })); if (this._options.length > 0) { this.requestUpdate(); } }); }
  render() {
    return html`
      <label>
        ${this.label ? html`<span>${this.label}</span>` : ""}
        <select .value=${this.value} ?disabled=${this.disabled} @change=${(e) => { this.value = e.target.value; this.dispatchEvent(new CustomEvent("change", { detail: this.value })); }}>
          ${this._options.map((opt) => html`<option value=${opt.value} ?selected=${this.value === opt.value}>${opt.label}</option>`)}
        </select>
      </label>
...
define("fable-select", FableSelect);
